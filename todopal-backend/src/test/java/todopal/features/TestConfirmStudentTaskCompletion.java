...
public class TestConfirmStudentTaskCompletion {
    ...
    public void there_is_a_class_task_called_that_is_worth_points(String taskName, String points) {
        final Task task = taskService.createTask(0, taskName, "", "chem", "mandatory", true, Integer.parseInt(points),
                LocalDate.now(), LocalDate.parse("9999-12-12"));
        taskService.createTaskContainer(0, LocalDate.parse("9999-12-12"), TaskStatus.PROGRESS,
                task.getTaskId());
    }
    ...
    public void a_task_is_marked_as_by_with_email(String taskName, String status, String studentName,
            String studentEmail) {
        try {
            final TaskContainer taskContainer = taskService.getTaskContainer(0);
    ...
    public void a_task_is_not_marked_as_done_by_with_email(String string, String string2, String string3) {
        // it's already at in done status
    }
    ...
    public void marks_as_officially_complete(String teacherName, String taskName) throws Exception {
        todopalRestController.approveTask("email@email.email", 0);
    }

    @Then("{string} will be marked as {string}")
    public void will_be_marked_as(String taskName, String taskStatus) {
        try {
            TaskContainer taskContainer = taskService.getTaskContainer(0);
            Assertions.assertEquals(TaskStatus.CLOSED, taskContainer.getStatus());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
